...
import { fileExists, getPackageVersion } from "./helpers.js";
...
if (!args.output) {
  console.warn(
    `warning: --output not specified, output will be generated to "${args.input}.decompressed"`
  );
}
...
const decompress = (input, output, offset) => {
  const handler = isNil(offset)
    ? explode()
    : transformSplitByIdx(offset, transformIdentity(), explode());
