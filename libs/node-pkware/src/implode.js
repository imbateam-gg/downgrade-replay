const { repeat, mergeRight, clone, last } = require("ramda");
const {
  DICTIONARY_SIZE1,
  DICTIONARY_SIZE2,
  DICTIONARY_SIZE3,
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < 0x10; i++) {
      for (let nCount2 = 0; nCount2 < 1 << ExLenBits[i]; nCount2++) {
        ...
        state.nChCodes[nCount] =
          (nCount2 << (LenBits[i] + 1)) | ((LenCode[i] & 0xffff00ff) * 2) | 1;
        nCount++;
      }
    }

    state.outputBuffer = Buffer.from([
      compressionType,
      state.dictionarySizeBits,
      0,
    ]);
    state.outBits = 0;
...
const outputBits = (state, nBits, bitBuffer) => {
  ...
  state.outputBuffer[state.outputBuffer.length - 1] |= getLowestNBits(
    8,
    bitBuffer << outBits
  );
  ...
  if (state.outBits > 8) {
    ...
    state.outputBuffer = appendByteToBuffer(
      getLowestNBits(8, bitBuffer),
      state.outputBuffer
    );
...
const processChunkData = (state) => {
  return new Promise((resolve, reject) => {
    if (state.inputBuffer.length > 0x1000 || state.streamEnded) {
      ...
      while (
        maxCycles-- > 0 &&
        (state.inputBuffer.length > 0 || !state.streamEnded)
      ) {
        let bytesToSkip = 0;
