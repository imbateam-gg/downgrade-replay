const { repeat, mergeRight, unfold, forEach } = require("ramda");
const {
  ERROR_INVALID_DATA,
...
const { flushBuffer } = require("./common.js");
...
const wasteBits = (state, numberOfBits) => {
  ...
  state.bitBuffer =
    ((state.bitBuffer >> state.extraBits) | (nextByte << 8)) >>
    (numberOfBits - state.extraBits);
...
const decodeNextLiteral = (state) => {
  ...
  if (state.compressionType === BINARY_COMPRESSION) {
    ...
    return wasteBits(state, 8) === PKDCL_STREAM_END
      ? LITERAL_STREAM_ABORTED
      : uncompressedByte;
  }
  ...
  return wasteBits(state, state.chBitsAsc[value]) === PKDCL_STREAM_END
    ? LITERAL_STREAM_ABORTED
    : value;
};
...
const decodeDistance = (state, repeatLength) => {
  ...
  if (repeatLength === 2) {
    ...
    distance =
      (distPosCode << state.dictionarySizeBits) |
      (state.bitBuffer & state.dictionarySizeMask);
    ...
    while ((nextLiteral = decodeNextLiteral(state)) < LITERAL_END_STREAM) {
      ...
      if (nextLiteral >= 0x100) {
        ...
        if (repeatLength > minusDistance) {
          const multipliedData = repeat(
            availableData,
            Math.ceil(repeatLength / availableData.length)
          );
